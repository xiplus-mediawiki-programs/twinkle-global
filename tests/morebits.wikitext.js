QUnit.module('MorebitsGlobal.wikitext');
QUnit.test('parseTemplate', assert => {
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate('Template: ' + makeTemplate(simple) + ' in text', 10), simple, 'Basic parameters');
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(makeTemplate(involved)), involved, 'Involved parameters');
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(whitespace), involved, 'Involved parameters with whitespace');
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(unnamedTemplate), unnamed, 'Unnamed and empty parameters');
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(makeTemplate(multiLevel)), multiLevel, 'Multiple levels');
	var parser = {
		name: 'toplevel',
		parameters: {
			named: '{{namedtop}}',
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(makeTemplate(parser)), parser, 'Parser function');
	...
	assert.deepEqual(MorebitsGlobal.wikitext.parseTemplate(makeTemplate(internal)), internal, 'Internal templates');
});

QUnit.test('MorebitsGlobal.wikitext.page', assert => {
	var text = '{{short description}}{{about}}[[File:Fee.svg]]O, [[Juliet|she]] doth {{plural|teach}} the torches to burn bright!';
	var page = new MorebitsGlobal.wikitext.page(text);
	assert.true(page instanceof MorebitsGlobal.wikitext.page, 'Correct instance');
	assert.strictEqual(page.getText(), text, 'Got text');

	// Throws
	assert.throws(() => new MorebitsGlobal.wikitext.page(text).insertAfterTemplates(), 'throws: no tag');
	assert.throws(() => new MorebitsGlobal.wikitext.page(text).insertAfterTemplates('tag'), 'throws: no regex');
	...
	tests.forEach((test) => {
		var page = new MorebitsGlobal.wikitext.page(test.input);
